<template>
  <div id="cloud">
            ...
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item style="font-size:12px;height:36px;text-align:center;">
                  <el-tooltip
                      :content="'空间大小：' + (totalSizes <= 1024 ? (totalSizes.toFixed(2) + 'MB') : ((totalSizes / 1024).toFixed(2) + 'GB')) + ' / ' + userSpaceSize / (1024 * 1024 * 1024) + 'GB'"
      ...
      <el-row class="menu">
        ...
        <el-col :span="1" v-show="flag">
          ...
          <span class="el-dropdown-link" v-else>
            <i class="el-icon-back el-icon--left"
               @click="goBack(folders[folders.length - 1].path, folders[folders.length - 1].depth)"></i>
          </span>
        </el-col>
        ...
        <el-col :span="width">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item icon="el-icon-folder-opened" v-for="folder in folders" :key="folder.id"
                                @click.native="getFolderFiles(folder.path, folder.depth + 1)"> {{ folder.filename }}
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </el-col>
        ...
        <el-col :span="6" style="text-align: right; float: right;">
          <el-dropdown trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item icon="el-icon-scissors" @click.native="moveFiles()">
                {{ moveFilesFlag ? '粘贴（' + moveFilesArray.length + '）' : '剪切' }}
              </el-dropdown-item>
              ...
              <el-dropdown-item icon="el-icon-refresh-right"
                                @click.native="updateFiles(nowFolder.path, nowFolder.depth + 1)">刷新
              </el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </el-col>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span v-if="judgeFileType(scope.row.filename) === 1" class="folder"
                    @click="accessFolder(scope.row.path, scope.row.depth + 1)"> {{ scope.row.filename }} </span>
              <span v-else
                    @click="downloadFile(scope.row.pathsUUID, scope.row.filename)"> {{ scope.row.filename }} </span>
            </template>
            ...
            <template slot-scope="scope">
              <el-container v-if="judgeFileType(scope.row.filename) === 1">
                <el-main :style="flag ? 'text-align:center;' : 'text-align:center;padding:0px;'">
                  <el-link type="primary" :style="flag ? 'margin-right:10px;' : ''"
                           @click="renameFile(scope.row.filename, scope.row.pathsUUID)"><i class="el-icon-edit"></i> 编辑
                  </el-link>
                  <el-link type="primary" @click="deleteFile(scope.row.filename, scope.row.pathsUUID)"><i
                      class="el-icon-folder-delete"></i> 删除
                  </el-link>
                </el-main>
              </el-container>
              <el-container v-else-if="judgeFileType(scope.row.filename) === 2">
                <el-main :style="flag ? 'text-align:center;' : 'text-align:center;padding:0px;'">
                  ...
                  <el-link type="primary" @click="renameFile(scope.row.filename, scope.row.pathsUUID)"><i
              ...
              <el-container v-else-if="judgeFileType(scope.row.filename) === 5">
                <el-main :style="flag ? 'text-align:center;' : 'text-align:center;padding:0px;'">
                  <el-link type="primary" :style="flag ? 'margin-right:10px;' : ''"
                           @click="showText(scope.row.pathsUUID)"><i class="el-icon-document"></i> 查看
                  </el-link>
                  <el-link type="primary" @click="renameFile(scope.row.filename, scope.row.pathsUUID)"><i
                      class="el-icon-edit"></i> 编辑
                  </el-link>
    ...
    <aplayer ref="aplayer" :audio="audio" :lrcType="0" fixed :mini="false" :listFolded="true" :autoplay="true"
             v-if="musicFlag"/>

    <div class="video-container" v-show="videoFlag">
      <d-player ref="dplayer" :options="options" class="video"
                :style="{width:videoWidth - 40 + 'px', height:videoHeight - 88 + 'px'}"></d-player>
...
//             console.log(value[0] + " " + path + " " + depth + " " + illegalFlag);
          this.$axios.post('http://' + this.baseHost + '/mycloud/pathsController/mkdir', this.$qs.stringify({
            userUUID: this.$store.state.userUUID,
            path: path,
            depth: depth,
            folder: value
    ...
    renameFile: function (filename, pathsUUID) {
      ...
      if (!moveFilesFlag) {
        ...
        this.$prompt('请输入' + tips + '的新名称：', '编辑', {
          ...
          if (!illegalFlag) {
            this.$axios.post('http://' + this.baseHost + '/mycloud/pathsController/renameFile', this.$qs.stringify({
              userUUID: this.$store.state.userUUID,
    ...
    deleteFiles: function () {
      ...
      if (this.filesSelection.length !== 0) {
        this.$confirm('提示：确定要彻底删除这' + this.filesSelection.length + '项么？该操作不可恢复！', '删除文件', {
          ...
          this.$axios.post('http://' + this.baseHost + '/mycloud/pathsController/delFiles', this.$qs.stringify({
            userUUID: this.$store.state.userUUID,
            lastPathsUUID: this.nowFolder.pathsUUID,
    ...
    if (this.moveFilesFlag) {
      ...
      this.$confirm('提示：确定将这' + this.moveFilesArray.length + '项移动到当前位置么？', '移动文件', {
        ...
        if (!flag && !this.repeatFlag) {
          this.$axios.post('http://' + this.baseHost + '/mycloud/pathsController/moveFiles', this.$qs.stringify({
            userUUID: this.$store.state.userUUID,
...
//         console.log(spaceSize);
      ...
      if (fileObj.size <= spaceSize) {
        ...
        this.$axios({
          method: 'post',
          url: 'http://' + this.baseHost + '/mycloud/pathsController/uploads',
          headers: {
...
//       console.log(this.images);
  ...
  playMusic: function (pathsUUID) {
    ...
    var musicArray = [];
    var cover = "../assets/image/71279418.png";
